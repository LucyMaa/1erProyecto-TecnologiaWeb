...
public class DReporte {
    ...
    public ArrayList<Reporte> listar(){
        ...
        try {
            String query = "SELECT plans.nombre,servicios.direccion,servicios.tipo,servicios.estado FROM servicios,plans where servicios.plan_id = plans.id";
            PreparedStatement pre = con.conectar().prepareStatement(query);
            ResultSet resp = pre.executeQuery();
            while(resp.next()){
                lista.add(new Reporte(resp.getString(1), resp.getString(2), resp.getString(3), resp.getString(4)));
            }
            pre.close();
        } catch (Exception e) {
